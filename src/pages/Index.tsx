  ...
  return (
    ...
    >
      ...
      >
        ...
        <a href="#main-content" className="sr-only focus:not-sr-only focus:absolute focus:top-4 focus:left-4 bg-black text-white p-4 rounded-lg z-[100]">
        ...
        >
          <img src={logo} alt="Adagio Logo" className="h-16 w-auto md:h-20" /> {/* Logo 4 veces más grande (de h-8 a h-16/20) */}
        </Link>
        ...
        <main id="main-content" className="flex-1 pt-40 pb-20 px-4 md:px-8 max-w-7xl mx-auto w-full space-y-8">
          ...
          <div className="text-center space-y-4 animate-fade-in-up [animation-delay:200ms] opacity-0 fill-mode-forwards">
             ...
             <p className="text-lg opacity-60 max-w-xl mx-auto">
               IA avanzada para el reconocimiento de habla atípica. 
             </p>
          </div>
          ...
          <div className="w-full max-w-4xl mx-auto animate-fade-in-up [animation-delay:300ms] opacity-0 fill-mode-forwards">
              <div className="bg-white/60 backdrop-blur-xl border border-white/40 rounded-[2.5rem] p-6 md:p-10 shadow-sm min-h-[400px] relative overflow-hidden">
                  ...
                  <TabsContent value="transcribe" className="mt-0 animate-in fade-in-50 slide-in-from-bottom-2 duration-500 focus-visible:outline-none">
                    <TranscribeView />
                  </TabsContent>
                  
                  <TabsContent value="train" className="mt-0 animate-in fade-in-50 slide-in-from-bottom-2 duration-500 focus-visible:outline-none">
                    <TrainView />
                  </TabsContent>
              </div>
          </div>
          ...
          <section className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-8 opacity-80 hover:opacity-100 transition-opacity max-w-4xl mx-auto">
             ...
             <Link to="/privacy-center" className="bg-[#0D0C1D] rounded-[2rem] p-6 text-[#F5F8DE] flex flex-col justify-between group hover:scale-[1.02] transition-transform cursor-pointer">
...
Aquí está la imagen actualizada con los cambios de diseño que solicitaste.
...
1.  **Guarda los cambios:** Copia y pega el código en tu `src/pages/Index.tsx`.
2.  **Verifica la apariencia:** Ahora el logo debería ser mucho más grande y estar en la esquina superior izquierda. La barra de herramientas flotante central tendrá los botones "Transcribir" y "Entrenar" separados, y el botón "Iniciar Sesión" se verá claramente.
3.  **Refina los componentes internos:** Es probable que los componentes `TranscribeView` y `TrainView` necesiten un pequeño ajuste para encajar perfectamente dentro del panel de cristal. Si ves que sus bordes o fondos no coinciden, dímelo y te ayudaré a adaptar esos también.

¿Qué te parece? ¿El logo y los botones ahora cumplen tus expectativas?