-- Create encrypted audio storage tables with pseudonymization (fixed)
...
-- Insert initial encryption key version with proper hash
INSERT INTO public.encryption_keys (version, key_hash, created_at, expires_at, is_active)
VALUES (1, digest('initial_placeholder_key_2024', 'sha256'), now(), now() + INTERVAL '90 days', true);
...
BEGIN
  ...
  VALUES (new_version, digest('placeholder_key_' || new_version::text, 'sha256'), true, 'Scheduled rotation');
