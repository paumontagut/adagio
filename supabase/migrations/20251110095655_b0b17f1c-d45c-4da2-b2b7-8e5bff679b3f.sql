-- Fix ambiguous column reference by using qualified names and parameter alias
DROP FUNCTION IF EXISTS public.get_participant_consents_with_token(text);
...
DECLARE
  admin_user_id uuid;
BEGIN
  -- Validate session token with fully qualified column names
